...
with open("src/components/DeviceDetail.tsx", "r", encoding="utf-8") as f:
    content = f.read()

old_block = """                                        const fullPrompt = `Here is my exact hardware configuration:
- Motherboard: ${mb}
- Display: ${disp}
...
new_block = """                                        let displayFlag = 'DISPLAY_ST7789'; // default
...
if old_block in content:
    ...
    # Also update the tooltip text below it to match the new behavior:
    content = content.replace('Click "COPY PROMPT" to grab the dynamically generated C++ layout string for this configuration.', 'Click "COPY PROMPT" to copy the exact configuration block needed at the top of your sketch.')
    
    with open("src/components/DeviceDetail.tsx", "w", encoding="utf-8") as f:
        f.write(content)
    print("Successfully replaced block!")
else:
    print("Could not find the exact old_block string match :(")
