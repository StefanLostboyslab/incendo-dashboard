...
#define FIRMWARE_VERSION "1.88"

OTAUpdate ota;
SFE_ST25DV64KC_NDEF st25dv;
...
String uniqueID, macStr, commandTopic, pingTopic, discoveryTopic, configTopic,
    modeTopic, nfcEventTopic, updateTopic, writeTopic;
...
void generateIdentity() {
  ...
  updateTopic = "incendo/devices/" + uniqueID + "/update";
...
bool programNDEF(NdefRecord* recs, uint8_t recCount, uint8_t *uid = NULL, uint8_t uidLength = 0) {
  ...
  if (hw_pn532 && pn532_initialized) {
    ...
    } else if (uidLength == 4 && uid != NULL) {
      // MIFARE Classic 1K Fallback (Single URL only)
      if (recCount > 0 && recs[0].type == "url") {
        String payloadUrl = recs[0].value;
        uint8_t ndefPrefix = 0x00;
        String cleanUrl = payloadUrl;
        if (payloadUrl.startsWith("https://www.")) { ndefPrefix = 0x02; cleanUrl = payloadUrl.substring(12); }
        else if (payloadUrl.startsWith("https://")) { ndefPrefix = 0x04; cleanUrl = payloadUrl.substring(8); }
  ...
  if (success) {
    tft.setTextColor(ST77XX_GREEN);
    tft.setTextSize(2);
    tft.println("Writing is DONE!");
    tft.setTextColor(ST77XX_WHITE);
    tft.setCursor(10, 120);
    tft.println("Remove NFC tag!");
  } else {
    tft.setTextColor(ST77XX_RED);
    tft.println("PN532: FAILED");
  }
  ...
  // 3. Handle Config/EPCIS Update
    ...
    if (!error) {
      ...
      if (doc.containsKey("hardwareModules")) {
        ...
        if (hw.containsKey("st25dv"))
          hw_st25dv = hw["st25dv"].as<bool>();
...
void executeOTA(String url) {
  ...
  if (ret == OTAUpdate::OTA_ERROR_NONE) {
    ...
    if (size > 0) {
      ...
      tft.println("Verifying & Flashing...");
      ota.verify();
      if (ota.update("/update.bin")) {
        tft.println("\nSUCCESS! Rebooting...");
        delay(2000);
        NVIC_SystemReset();
      } else {
...
void setup() {
  ...
  if (hw_st25dv) {
    if (st25dv.begin(Wire)) {
      Serial.println("Found SparkFun ST25DV EEPROM module");
      st25dv_initialized = true;
    } else {
      Serial.println("Warning: ST25DV not found!");
      tft.fillScreen(ST77XX_RED);
      tft.setCursor(10, 100);
      tft.setTextColor(ST77XX_WHITE);
      tft.setTextSize(2);
      tft.println("EEPROM MISSING");
      delay(2000);
    }
  }
  ...
  if (hw_pn532) {
    ...
    if (!versiondata) {
      Serial.println(
          "Warning: PN532 not found during setup check. Continuing anyway.");
      ...
      tft.setTextColor(ST77XX_WHITE);
      tft.setTextSize(2);
      tft.println("NFC NOT FOUND");
      tft.setTextSize(1);
      tft.setCursor(10, 140);
      tft.println("Will retry if conf updated");
      delay(2000);
    } else {
      Serial.print("Found PN532 chip: PN5");
...
void loop() {
  ...
  if (currentMode == "READ" && hw_pn532 && pn532_initialized &&
    ...
    if (nfc.readPassiveTargetID(PN532_MIFARE_ISO14443A, uid, &uidLength, 50)) {
      ...
      String fullUrl = "";
      if (prefixByte < (sizeof(URI_PREFIX_MAP) / sizeof(URI_PREFIX_MAP[0]))) {
        fullUrl = String(URI_PREFIX_MAP[prefixByte]) + record1;
      } else {
        fullUrl = record1;
      }
      
      // Defensively strip physical `#` terminating buffer artifacts
      if (fullUrl.endsWith("#")) {
